...
(define-empty-tokens ets (+ - * / % ^ < = : LPAREN RPAREN IF THEN ELSE LET IN
                            LETEND LAMBDA ARROW BOOLT NUMT DOT APP REC END FAIL))
  ...
  (parser
    (grammar (TOP
             ...
             (F
               ...
               ((LET REC VAR VAR : TYPE = E IN E LETEND)
                (prec LET) (let-rec-in-node $3 $4 $6 $8 $10))
               )
             (TYPE
               ((BOOLT) (boolt-node))
               ((NUMT) (numt-node))
...
(define (eval e env)
  (define (eval* e env)
    (match e
           ...
           [(struct let-rec-in-node (f x t e1 e2))
            (let ([fn (closure e1 (void) x)])
              (set-closure-env! fn (cons `(,f . ,fn) env)) ; ugh
              (eval* e2 (cons `(,f . ,fn) env)))]
...
(module+ test ;; no spec so there can't be bugs.  thanks ernst!
         ...
         (check-equal? (try-eval "let rec fact n : num->num = if n < 2 then 1 else n * fact (n - 1) in fact 5 end") "120")
         (check-eq? (try-eval "let rec fact n : num->bool = if n < 2 then 1 else n * fact (n - 1) in fact 5 end") #f)
         (check-equal? (try-eval "let rec isprime n : num->bool =
                                 let rec aux m : num->bool =
                                 if m < 2 then true else if n % m < 1
                                 then false else aux (m - 1) in aux (n - 1) end
                                 in isprime 99 end") "false")
         (check-equal? (try-eval "let rec isprime n : num->bool =
                                 let rec aux m : num->bool =
                                 if m < 2 then true else if n % m < 1
                                 then false else aux (m - 1) in aux (n - 1) end
                                 in isprime 83 end") "true")
         )
